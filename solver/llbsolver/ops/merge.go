...
// TODO parallelism limiter for inefficient case?
...
func (m *mergeOp) CacheMap(ctx context.Context, group session.Group, index int) (*solver.CacheMap, bool, error) {
	...
	for i := range m.op.Inputs {
		// TODO once there is fs metadata support these can be updated with funcs that only pull+operate on metadata
...
func (m *mergeOp) Exec(ctx context.Context, g session.Group, inputs []solver.Result) ([]solver.Result, error) {
	...
	mergedRef, err := m.worker.CacheManager().Merge(ctx, refs) // TODO any opts? Description? Other stuff?
	if err != nil {
		return nil, err
	}
