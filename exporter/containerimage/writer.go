...
func (ic *ImageWriter) commitDistributionManifest(ctx context.Context, ref cache.ImmutableRef, config []byte, remote *cache.Remote, oci bool, inlineCache []byte) (*ocispec.Descriptor, error) {
	if len(config) == 0 {
		var err error
	...
	remote, history = normalizeLayersAndHistory(remote, history, ref)
...
func normalizeLayersAndHistory(remote *cache.Remote, history []ocispec.History, ref cache.ImmutableRef) (*cache.Remote, []ocispec.History) {
