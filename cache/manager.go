	...
	for _, si := range items {
		...
		if em := getEqualMutable(si); em != "" {
			...
			if emSi, exists := cm.md.Get(getEqualMutable(si)); exists {
				if getSnapshotID(si) == "" {
					queueSnapshotID(si, getSnapshotID(emSi))
				}
				if GetDescription(si) == "" {
					queueDescription(si, GetDescription(emSi))
				}
				if getSize(si) <= 0 && getSize(emSi) > 0 {
					setSize(si, getSize(emSi))
				}
				if err := si.Commit(); err != nil {
					return errors.Wrapf(err, "failed to migrate equalMutable %s to ref %s", em, si.ID())
				}
			}
		}
...
func (cm *cacheManager) getRecord(ctx context.Context, id string, opts ...RefOption) (cr *cacheRecord, retErr error) {
	...
	if !rec.isFinalized {
		...
		var err error
		if info, err = rec.cm.Snapshotter.Stat(ctx, getSnapshotID(rec.md)); errdefs.IsNotFound(err) {
