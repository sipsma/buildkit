...
func (cm *cacheManager) GetByBlob(ctx context.Context, desc ocispec.Descriptor, parent ImmutableRef, opts ...RefOption) (ir ImmutableRef, rerr error) {
	...
	for _, si := range sis {
		...
		if err := setImageRefMetadata(ref.storageItem, opts...); err != nil {
	...
	if err := initializeMetadata(rec.storageItem, parentID, opts...); err != nil {
		return nil, err
	}

	if err := setImageRefMetadata(rec.storageItem, opts...); err != nil {
...
func (cm *cacheManager) getRecord(ctx context.Context, id string, opts ...RefOption) (cr *cacheRecord, retErr error) {
	...
	if err := initializeMetadata(rec.storageItem, md.getParent(), opts...); err != nil {
		return nil, err
	}
...
func CachePolicyRetain(m *storageItem) error {
	return m.QueueCachePolicyRetain()
}

func CachePolicyDefault(m *storageItem) error {
	return m.QueueCachePolicyDefault()
}

func WithDescription(descr string) RefOption {
	return func(m *storageItem) error {
...
type imageRefOption func(m *storageItem) error

// WithImageRef appends the given imageRef to the cache ref's metadata
func WithImageRef(imageRef string) RefOption {
	return imageRefOption(func(m *storageItem) error {
		return m.appendImageRef(imageRef)
	})
}

func setImageRefMetadata(m *storageItem, opts ...RefOption) error {
	for _, opt := range opts {
...
func initializeMetadata(m *storageItem, parent string, opts ...RefOption) error {
	...
	for _, opt := range opts {
		if fn, ok := opt.(func(*storageItem) error); ok {
			if err := fn(m); err != nil {
				return err
			}
		}
