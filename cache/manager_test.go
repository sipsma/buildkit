...
func newCacheManager(ctx context.Context, opt cmOpt) (co *cmOut, cleanup func() error, err error) {
	...
	cm, err := NewManager(ManagerOpt{
		Snapshotter:    snapshot.FromContainerdSnapshotter(ctx, opt.snapshotterName, containerdsnapshot.NSSnapshotter(ns, mdb.Snapshotter(opt.snapshotterName)), nil, lm, false),
		MetadataStore:  md,
		ContentStore:   store,
		LeaseManager:   lm,
			...
			eg.Go(func() error {
				...
				for i, desc := range remote.Descriptors {
					...
					if compressionType == compression.EStargz {
						if digest.Digest(curRef.getBlob()) == desc.Digest {
							esgzRefsMu.Lock()
...
func TestMergeOp(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		...
		target, err := lm.Mount()
		require.NoError(t, err)
		err = fstest.Apply(fstest.CreateFile(strconv.Itoa(i), []byte(strconv.Itoa(i)), 0777)).Apply(target)
		require.NoError(t, err)
		err = lm.Unmount()
		require.NoError(t, err)
		snap, err := active.Commit(ctx)
