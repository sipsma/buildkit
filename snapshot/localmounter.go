...
func withTempMount(ctx context.Context, mounts []mount.Mount, f func(root string, isDirect bool) error) error {
	if len(mounts) == 0 {
		...
		return f(tmpdir, true)
	}
	if len(mounts) == 1 {
		mnt := mounts[0]
		if mnt.Type == "bind" || mnt.Type == "rbind" {
			return f(mnt.Source, true)
		}
	}
...
func withRWDirMount(ctx context.Context, mounts []mount.Mount, f func(root string, isDirect bool) error) error {
	if len(mounts) != 1 {
